...
### データを平日と休日に分割
KSQLでデータを加工するには、STREAMオブジェクトとして宣言する必要があります。構文は```CREATE STREAM ...```となります。
...
上記の様にSELECTで対象STREAMを指定の上、そのまま新しいSTREAMとして定義しています。この構文はCSAS (Create Stream As Selectの略) と呼ばれる提携構文であり、KSQLによるストリーム処理では頻出の表現です。
...
結果として、新しく作成したSTREAMには付随するTopicが定義されており、このTopicにクエリ結果が流れるようになります。結果として今回は新しく```weekdays```と```weekends```という2つのTopicが生成され、それぞれに条件に合ったレコードのみ流れていることが確認できます。
