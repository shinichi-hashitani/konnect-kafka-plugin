...
### データを平日と休日に分割
KSQLでデータを加工するには、Streamオブジェクトとして宣言する必要があります。構文は```CREATE STREAM ...```となります。TopicもStreamもイベントをを管理するユニットであり違いが分かりにくいかも知れません。基本的ににTopicとStreamは同じものを指しています。Topicが物理的な定義、Streamが論理的な定義であり、少なくともKSQLは論理的な処理（クエリ言語）である為、Streamの定義が必要となります。
...
Topic内の型の定義や参照元Topic名、フォーマット等を指定します。Stream化されたデータはKSQL上でクエリ操作可能になります。今回は```source```にあるTopicから条件を指定したメッセージのみ別のTopicにコピーしたいので：
...
上記の様にSELECTで対象Streamを指定の上、そのまま新しいStreamとして定義しています。この構文はCSAS (Create Stream As Selectの略) と呼ばれる定型構文であり、KSQLによるストリーム処理では頻出の表現です。

最後の```EMIT CHANGES```という構文がSQL的には少し妙ですが、これは結果として出力したStreamのイベントを次の処理に繋げる時には必要なKSQL独自の構文です。基本的にCSAS構文には必ず```EMIT CHANGES```が必要だと思ってください。

上記の定義をKSQLに送ると、必要に応じて追加のTopicの生成を含めてKSQLが自動的に作成します。今回はConfluent Platformを使っており、Control CenterにKSQL用のUIエディタがあるのでそれを利用します。

![Control Center - ksqlDB Editor](/resources/control-center-ksql-csas.png)

ksqlフォルダ配下にある[1-source_events.sql](/ksql/1-source_events.sql)、[2-wekdays_stream.sql](/ksql/2-weekdays_stream.sql)、[3-weekends_stream.sql](/ksql/3-weekends_stream.sql)の内容をそれぞれエディタにコピーの上、```Run Query```ボタンをクリックして実行してください。結果、Flowを確認すると定義したフローが可視化されます。

![Control Center - ksqlDB Flow](/resources/control-center-csas-flow.png)

結果として、新しく作成したStreamには付随するTopicが定義されており、このTopicにクエリ結果が流れるようになります。今回は新しく```weekdays```と```weekends```という2つのTopicが生成され、それぞれに条件に合ったレコードのみ流れていることが確認できます。

![Control Center - Topic - Added 2 new topics](/resources/control-center-topics-csas.png)
![Control Center - Topic - weekdays](/resources/control-center-csas-topic.png)

> [!NOTE]
> この際、```auto.offset.reset```の値を```earliest```に変更してください。これによりこれまで```source```に送ったデータの最初から読み込んで処理されます。

### ストリームのマテリアライズ
Streamは常にデータが流れており、継続的に取り込まないと状態のような情報を得る事ができません。これはリレーショナルモデルのテーブルとは概念が大きく異なります。一方、Streamをマテリアライズする事により、テーブルの様にその時の状態を定義する事も可能です。

KSQLではその概念モデルをTableと呼びます。例えば```WEEKDAYS_STREAM```に対して特定の項目の集約値を取りたい場合は：
...
の様な定義となります。ほぼほぼSQLのクエリと同じとなります。またこの構文も

また、先程のCSASの時と同様、この構文はCTAS (Create Table As Select) と呼ばれており定型構文です。```CREATE STREAM```の時と同様、[4-weekdays_table.sql](/ksql/4-weekdays_table.sql)と[5-weekends_table.sql](/ksql/5-weekends_table.sql)もksqlDBエディタを使って適用します。

![Control Center - Flow with Tables](/resources/control-center-flow-with-table.png)

実際のデータアクセスですが、KSQLに対して普通に

```sql
SELECT * 
FROM WEEKDAYS_SCORE_STATS;
```

のようにクエリする事が可能です。このテーブルはStreamをマテリアライズしたものなので、常時その時点での集計値をリアルタイムに取る事が可能です。残念ながらConfluent Control CenterのksqlDBエディタは編集用のツールなので参照クエリの実行には向きません。今回は別途取得したKSQLのバイナリを利用します。

ダイナミックなステート変更を確認するために、常時メッセージを送り続けるようにします。Insomniaで先程のクエリを常時実行するように設定します。(0.5秒が最短インターバル)

![Insomnia - Run on Interval](/resources/insomnia-repeat-on-interval.png)
...
------------------------------------------------------------------------------------
 WEEKDAYS_SCORE_STATS | WEEKDAYS_SCORE_STATS | KAFKA      | JSON         | false
 WEEKENDS_SCORE_STATS | WEEKENDS_SCORE_STATS | KAFKA      | JSON         | false
------------------------------------------------------------------------------------
ksql> SELECT * FROM WEEKDAYS_SCORE_STATS;
+------------------------------+------------------------------+------------------------------+------------------------------+
|DAY_KEY                       |MIN_SCORE                     |MAX_SCORE                     |AVG_SCORE                     |
+------------------------------+------------------------------+------------------------------+------------------------------+
|Friday                        |0                             |100                           |50.94736842105263             |
|Monday                        |0                             |99                            |53.04081632653061             |
|Thursday                      |4                             |100                           |54.78                         |
|Tuesday                       |3                             |96                            |45.696969696969695            |
|Wednesday                     |0                             |97                            |50.72222222222222             |
Query terminated
```

この状態でも絶えずメッセージは送られており、マテリアライズされたビューもリアルタイムで更新されています。少し時間を経過したのちに実行すると：
...
|DAY_KEY                       |MIN_SCORE                     |MAX_SCORE                     |AVG_SCORE                     |
+------------------------------+------------------------------+------------------------------+------------------------------+
|Friday                        |0                             |100                           |51.38297872340426             |
|Monday                        |0                             |99                            |52.088235294117645            |
|Thursday                      |1                             |100                           |53.559322033898304            |
|Tuesday                       |3                             |100                           |49.36082474226804             |
|Wednesday                     |0                             |99                            |49.421487603305785            |
Query terminated
```
